...
⚠️ **本家UTAUでは使えません**: この機能はOpenUtauとUTAletでのみ動作します。

Voice Colorを使うと、1つの音源で「通常の声」「強い声」など複数の声色を切り替えられます。

大規模な音源でなければ使用しません。基本的な多音階設定のみで十分な場合は、この機能はスキップして構いません。

### Voice Colorの操作方法

右側のパネルに、Voice Color管理用のコントロールがあります。

#### 編集対象のVoice Colorを切り替える

一番上のセレクトボックスで、現在編集中のVoice Colorを選択できます。選択したVoice Colorのprefix/suffix設定が表示されます。
...
- 前のページ: [9.インストール設定](/docs/tutorial-ja-09-インストール設定--docs)
